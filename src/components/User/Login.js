...
const required = (value) => {
    if (!value) {
        return (
            <div className="text-xs tracking-wide text-red-600">Thông tin không được để trống</div>
        )
    }
}
...
const minLength = (value) => {
    if (value.trim().length < 4) {
        return <small className="text-xs tracking-wide text-red-600">Mật khẩu phải có ít nhất 4 ký tự</small>;
    }
}
                ...
                <Form action="" onSubmit={handleLogin} ref={formRef}>
                    <div className="mt-4">
                        ...
                        <div className="flex justify-between items-center mt-4 mb-2">
                            <button className="px-7 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-700 w-full" disabled={loading}>
                                {loading && (<svg className=" animate-spin h-5 w-5 inline-block mr-3 text-white" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                        ...
                        {message && <div
                            ...
                            <div className="flex text-center">
                                <div className="py-1">
                                    <svg className="fill-current h-5 w-5 text-teal-500 mr-3"
